...
class SplitLayer(nn.Module):
    def __init__(self, input_size, hidden_size, batch_size):
        ...
        W1 = nn.Parameter(torch.randn(hidden_size + input_size + 2,
                          hidden_size))
        self.register_parameter('W1', W1)
        W2 = nn.Parameter(torch.randn(hidden_size + input_size + 2,
                          hidden_size))
...
class Split(nn.Module):
    ...
    def forward(self, e, input, mask, scale=0):
        ...
        noise = Variable(torch.zeros(input.size())).type(dtype)
        noise.data.uniform_()
        input = torch.cat((input_n, noise), 2)
        # input_n = input * mask_inp
        for i, layer in enumerate(self.layers):
            hidden = layer(input, hidden, Phi, Nh)
